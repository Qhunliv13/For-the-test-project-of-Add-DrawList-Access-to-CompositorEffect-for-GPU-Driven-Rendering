...
class_name FinalComputeRenderDemo
...
func setup():
	...
	var time_bytes = PackedFloat32Array([0.0]).to_byte_array()
	time_buffer = rd.storage_buffer_create(time_bytes.size(), time_bytes)
	
	var compute_file = load("res://final_compute.glsl")
	...
	var u_time = RDUniform.new()
	u_time.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER
	u_time.binding = 1
	u_time.add_id(time_buffer)
	compute_uniforms.append(u_time)
	
	compute_uniform_set = rd.uniform_set_create(compute_uniforms, compute_shader, 0)
	
	var render_file = load("res://final_render.glsl")
	...
	dummy_vertex_array = rd.vertex_array_create(3, vf, [dummy_vertex_buffer])
...
func update_buffers(render_data: RenderData):
	...
	var time_bytes = PackedFloat32Array([current_time]).to_byte_array()
	rd.buffer_update(time_buffer, 0, time_bytes.size(), time_bytes)
